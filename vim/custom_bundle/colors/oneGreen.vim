...
if has('gui_running') || &t_Co == 88 || &t_Co == 256
  ...
  let s:syntax_bg     = s:main
  let s:syntax_gutter = s:alt
  let s:syntax_cursor = s:alt " tab line bg

  let s:vertsplit    = s:alt
  let s:special_grey = s:alt
  let s:pmenu        = s:alt " popup menu
...
"}}}
  " Public API --------------------------------------------------------------{{{
  function! oneGreen#highlight(group, fg, bg, attr)
